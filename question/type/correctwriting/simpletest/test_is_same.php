...
class qtype_correctwriting_sequence_analyzer_token_base_is_same extends UnitTestCase {
    ...
    public function test_equal_tokens() {
        $answer=new qtype_correctwriting_token_base("type","value",true,null);
        $response=new qtype_correctwriting_token_base("type","value",false,null);
    ...
    public function test_equal_tokens_is_null() {
        $answer=new qtype_correctwriting_token_base("type",null,true,null);
        $response=new qtype_correctwriting_token_base("type",null,false,null);
        $this->assertTrue( $answer->is_same($response) );
    }
    //Test when tokens are not equal and values are different
    public function test_inequal_values() {
        $answer=new qtype_correctwriting_token_base("type",null,true,null);
        $response=new qtype_correctwriting_token_base("type","test",false,null);
    ...
    public function test_inequal_types() {
        $answer=new qtype_correctwriting_token_base("type","test",true,null);
        $response=new qtype_correctwriting_token_base("type2","test",false,null);
        $this->assertFalse( $answer->is_same($response) );
    }
